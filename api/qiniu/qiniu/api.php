...
$oauthurl = base64_decode($_GPC['host']);
$visitorsip =  base64_decode($_GPC['visitorsip']);
$webname =  base64_decode($_GPC['webname']);

$sql = 'SELECT * FROM ' . tablename('fm_api_media') . ' WHERE `oauthurl`=:oauthurl';
$pars = array();
...
if ($oauth) {
	$q = pdo_fetch('SELECT * FROM ' . tablename('fm_api_media') . ' WHERE `oauthurl`=:oauthurl AND `visitorsip`=:visitorsip AND `webname`=:webname', array(':oauthurl'=>$oauthurl,':visitorsip'=>$visitorsip,':webname'=>$webname));
	if (!$q) {
		pdo_update('fm_api_media',array('visitorsip'=>$visitorsip, 'webname'=>$webname),array('oauthurl'=>$oauthurl));
	}
	...
	if ($oauth['qiniucloud']) {
		if ($_GPC['fmimages']) {
			...
			$fmdata = array(
				"success" => -1,
				"msg" => '多媒体上传云存储为高级功能，需付费使用，如有需要，请联系我们（QQ：513200958）！',
	...
	pdo_insert('fm_api_media_list', array('oauthurl'=>$oauthurl, 'visitorsip'=>$visitorsip, 'createtime'=>time()));
	$fmdata = array(
		"success" => -1,
		"msg" => '你所使用的为高级功能，需付费使用，如有需要，请联系我们（QQ：513200958）！',
