...
function attachment_qiniu_auth($key, $secret,$bucket) {
	...
	list($ret, $err) = $uploadmgr->putFile($token, 'weizan.ico', ATTACHMENT_ROOT.'images/global/weizan.ico');
	if ($err !== null) {
		$err = (array)$err;
...
function attachment_cos_auth($bucket,$appid, $key, $secret) {
	...
	$uploadRet = \Qcloud_cos\Cosapi::upload($bucket, ATTACHMENT_ROOT.'images/global/weizan.ico', '/weizan.ico','',3 * 1024 * 1024, 0);
	if ($uploadRet['code'] != 0) {
