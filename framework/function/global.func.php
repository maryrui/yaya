...
function printer($senddata,$modulename='',$id='',$mid=''){
	global $_W;
	if($id){
		$data=pdo_getall('printer',array('id'=>$id,'uniacid'=>$_W['uniacid']));
	}else if($modulename){
		$data=pdo_getall('printer',array('module'=>$modulename,'uniacid'=>$_W['uniacid']));
	}else{
		$data=pdo_getall('printer',array('isdefault'=>1,'uniacid'=>$_W['uniacid']));
	}
	if(empty($data) ||empty($senddata)){
		return 'error1';
	}
	if($mid){
		$printep=pdo_get('printep',array('id'=>$mid,'uniacid'=>$_W['uniacid']));
	}else if($modulename){
		$printep=pdo_get('printep',array('module'=>$modulename,'uniacid'=>$_W['uniacid']));
	}else{
		$printep=pdo_get('printep',array('defaul'=>1,'uniacid'=>$_W['uniacid']));
	}
	if(empty($printep) || !is_array($senddata)){
		$sendcontent=$senddata;
	}else{
		$sendcontent=base64_decode($printep['content']);
		foreach($senddata as $key=>$send){
			$sendcontent=str_replace('{'.$key.'}',$send,$sendcontent);
	    }
	}
	load()->classs('jinyun');
	$pr=new JinYunZN;
	...
	foreach($data as $p){
		$sendcontent=$p['top'].'\n'.$sendcontent.'\n'.$p['bottom'];
		$return=$pr->SendSocketJson($p['token'],$p['apikey'],$p['dtuid'],$p['imei'],$sendcontent);
		if($return == 117){
			$num=$num+1;
		}
	}
...
function sendsms($mobile,$content=array(),$mobanid='',$msg=''){
	//函数介绍：该函数为全局短信发送函数，传入参数依次为发送手机号（可多个，用逗号隔开），变量数组，短信模版ID（可选），短信接口参数（可选，为空默认调用当前公众号全局短信接口参数），目前仅适用阿里大于短信平台。
	global $_W;
	...
	$req->setSmsFreeSignName($msg['qianming']);
...
function export2excel($header,$data,$filename){
	$html="\xEF\xBB\xBF";
