...
load()->model('cache');
load()->func('file');
$dos = array('installed', 'check', 'prepared', 'install', 'upgrade', 'uninstall', 'designer', 'permission', 'batch-install', 'info', 'recycle','bak');
...
if ($do == 'bak') {
	$name=trim($_GPC['id']);
	$mdir=IA_ROOT .'/data/modulesbak/'.$name;
	if(!is_dir($mdir)){
		message('没有可回滚的模块版本，或者上次更新没有修改文件，无需回滚！');
	}
	$time=0;
	$bak = '';
	...
	$zip = new ZipArchive();
	$zip->open($mdir.'/'.$bak);
	$zip->extractTo(IA_ROOT .'/addons/'.$name);
	$zip->close();
	@unlink($mdir.'/'.$bak);
	$bakr=strrchr($bak,'.');
	$version=str_replace($bakr,'',$bak);
	pdo_update('modules',array('version'=>$version),array('name'=>$name));
	message('成功将模块版本回滚至上次更新前的版本！');
}
if ($do == 'recycle') {
	$operate = $_GPC['op'];
	$name = trim($_GPC['name']);
...
if($do == 'install') {
	...
	if (empty($manifest)) {
		...
		$pars['install'] = 1;
		$pars['token'] = random(20);
		cache_write($modulename.'token',$pars['token']);
	    $dat = cloud_request(ADDONS_URL.'/gateway.php', $pars);
	    $dat = unserialize(base64_decode($dat['content']));
        if($dat['state']){
	        message($dat['content']);
		...
		if(is_array($files)){
			...
			foreach($files as $key=>$file){
				$f=IA_ROOT . '/data/'.$modulename.$key.$file;
		...
		$zip->close();
		@unlink($temp);
		$manifest = ext_module_manifest_parse($dat['manifest']);
	}
	if (empty($manifest)) {
	...
	if (pdo_insert('modules', $module)) {
		...
		if (empty($module_subscribe_success)) {
			message('模块安装成功, 请按照【公众号服务套餐】【用户组】来分配权限！模块订阅消息有错误，系统已禁用该模块的订阅消息，详细信息请查看 <div><a class="btn btn-primary" style="width:80px;" href="' . url('extension/subscribe/subscribe') . '">订阅管理</a> &nbsp;&nbsp;<a class="btn btn-default" href="' . url('extension/module1') . '">返回模块列表</a></div>', '', 'tips');
		} else {
			message('模块安装成功, 请按照【公众号服务套餐】【用户组】来分配权限！', url('extension/module1'), 'success');
		}
	} else {
...
if ($do == 'uninstall') {
	...
	if ($module['isrulefields'] && !isset($_GPC['confirm'])) {
		...
		message('模块已放入回收站！', url('extension/module1'), 'success');
	}
}
...
if($do == 'upgrade') {
	...
	$module = pdo_fetch("SELECT name, version FROM ".tablename('modules')." WHERE name = '{$id}'");
	$pars['method'] = 'module1.info';
	$pars['module'] = $id;
	...
	$dat = unserialize(base64_decode($dat['content']));
    if($dat['state']){
	    message($dat['content'], '', 'ajax');
    }
		message($dat, '', 'ajax');
	}
	$sql = 'SELECT * FROM ' . tablename('modules') . ' WHERE `name`=:name';
	$module = pdo_fetch($sql, array(':name' => $id));
	$pars['method'] = 'module1.build';
	$pars['module'] = $id;
	if(!empty($module)) {
	...
	$dir = IA_ROOT . '/addons/' . $id;
	//更新文件
	$upfiles=$files = array();
	if(!empty($dat['files'])) {
		$mdir=IA_ROOT .'/data/modulesbak/'.$id;
		if(!is_dir($mdir)){
			mkdirs($mdir);
		}
	...
	//更新数据库
	//$sqls = array();
	//if(!empty($dat['schemas']) && !empty($module)) {
	//	load()->func('db');
	...
	//模块文件校验
	//$filecheck=file_tree($dir,array(),array('.php'));
	//print_r($filecheck);echo '<br>';
	//$check=$unknown=array();
	//foreach($dat['files'] as $df){
	//	$check[$df['path']]=$df['checksum'];
	//}
	...
	if(strrchr($manifest['install'],'.')=='.php'){
		message('备用更新异常，请移步系统默认更新接口更新该模块！',url('extension/module'));
	}
	$dbs=update($manifest['install']);
	foreach($dbs as $tb){
		if(!pdo_tableexists($tb['name'])) {
			pdo_query($tb['install']);
		}
		else{
			$local=pdo_fetch('show create table '.tablename($tb['name']));
			$local=updatelocal($local['Create Table']);
			foreach($tb['fields'] as $field){
				if(!pdo_fieldexists($tb['name'], $field['name'])) {
					pdo_query("ALTER TABLE ".tablename($tb['name'])." ADD ".$field['value'].";");
				}elseif($local['fields'][$field['name']]['value'] !=$field['value']){
					pdo_query("ALTER TABLE ".tablename($tb['name'])." CHANGE `".$field['name']."` ".$field['value'].";");
				}
			}
			foreach($tb['indexs'] as $index){
				if(!strexists($index['value'],'PRIMARY KEY')){
					if(!pdo_indexexists($tb['name'], $index['name'])) {
						pdo_query("ALTER TABLE ".tablename($tb['name'])." ADD ".$index['value'].";");
					}elseif($local['indexs'][$index['name']]['value'] !=$index['value']){
						pdo_query("ALTER TABLE ".tablename($tb['name'])."DROP INDEX `".$index['name']."`, ADD ".$index['value'].";");
					}
				}
			}
		}
	...
	//	message('模块更新成功！ <br> 系统检测到您的模块目录存在异常文件！请确认是否是自己二次开发所添加的文件.<div><a class="btn btn-primary" href="' . url('system/filecheck/checkmodule') . '">现在查看</a>&nbsp;&nbsp;&nbsp;<a class="btn btn-default" href="' . url('extension/module1/') . '">返回模块列表</a></div>', '', 'success');
	//}
	message('模块更新成功！ <br> 由于数据库更新, 可能会产生多余的字段. 你可以按照需要删除.<div><a class="btn btn-primary" href="' . url('system/database/trim') . '">现在去删除</a>&nbsp;&nbsp;&nbsp;<a class="btn btn-default" href="' . url('extension/module1/') . '">返回模块列表</a></div>', '', 'success');
}
    ...
    foreach($installs as $row){
	    ...
	    foreach($temp2 as $key => $value){
		    ...
		    if(substr($value,0,1)=="`"){
			    ...
			    $name=$temp['1'];
				if(!empty($name)){
					$indexs[]=array('name'=>$name,'value'=>$value);
				}
		    }
	    }
...
function updatelocal($row){
	    ...
	    foreach($temp2 as $key => $value){
		    ...
		    if(substr($value,0,1)=="`"){
			    $temp=explode('`',$value);
			    $name=$temp['1'];
			    $fields[$name]=array('name'=>$name,'value'=>$value);
		    }
		    else{
				$temp=explode('`',$value);
			    $name=$temp['1'];
				if(!empty($name)){
					$indexs[$name]=array('name'=>$name,'value'=>$value);
				}
		    }
	    }
