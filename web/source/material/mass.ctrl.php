...
if($do == 'post') {
	...
	foreach($mass as $row) {
		...
		$extra=date('y-m-d',$data['sendtime']);
		$extra=$_W['uniacid'].$extra;
		$insert_id = pdo_insertid();
		$url=$_W['siteroot'].'web/index.php?c=cron&a=entry&do=mass&id='.$insert_id;
		...
		$cron = array(
			...
			'status'=>1,
			'type' => 'ds',
			'value' => strtotime($row['time'] . " {$row['clock']}"),
			'open'=>1,


		);
		$old=pdo_get('core_cron',array('extra'=>$extra));
		if(!empty($old)){
			$cron['id']=$old['cloudid'];
			$status = cloud_cron_update($cron);
			if($status!='SUCCESS') {
				$message .= "{$row['time']}的群发任务同步到云服务失败,请手动同步<br>";
				$cron_status = 1;
			}else{
				$cron['id'] = $old['id'];
				pdo_update('core_cron', $cron, array('id' => $old['id'], 'uniacid' => $_W['uniacid']));
				pdo_update('mc_mass_record',array('cron_id'=>$old['cloudid']),array('id'=>$insert_id));
			}
		}else{
			$status = cloud_cron_update($cron);
			if(!is_numeric($status)) {
				...
				$cron['cloudid'] = intval($status);
				$cron['uniacid'] = $_W['uniacid'];
				$cron['extra']=$extra;
				pdo_insert('core_cron', $cron);
				pdo_update('mc_mass_record',array('cron_id'=>$cron['cloudid']),array('id'=>$insert_id));
			}
		}
...
if($do == 'cron') {
	...
	$extra=date('y-m-d',$record['sendtime']);
	$extra=$_W['uniacid'].$extra;
	$url=$_W['siteroot'].'web/index.php?c=cron&a=entry&do=mass&id='.$id;
	$url=base64_encode($url);
	$cron = array(
			'name' =>"微信群发任务",
			'url' => $url,
		...
		if(!empty($old)){
			...
			if($status!='SUCCESS') {
				...
				$cron['id'] = $old['id'];
				pdo_update('core_cron', $cron, array('id' => $old['id'], 'uniacid' => $_W['uniacid']));
				pdo_update('mc_mass_record',array('cron_id'=>$old['cloudid']),array('id'=>$record['id']));
			}
		}else{
			$status = cloud_cron_update($cron);
			if(!is_numeric($status)) {
				...
				$cron['extra']=$extra;
				pdo_insert('core_cron', $cron);
				pdo_update('mc_mass_record',array('cron_id'=>$cron['cloudid']),array('id'=>$record['id']));
			}
		}
