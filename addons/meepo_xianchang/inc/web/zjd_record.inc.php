...
if($op == 'list') {
	...
	$list = pdo_fetchall($sql, array(':weid'=>$weid,':rid'=>$id,':type'=>2));
	...
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->lottory_user_table) . " WHERE  rid=:rid AND weid=:weid AND type=:type", array(':rid' =>$rid,':weid'=>$weid,':type'=>2));
	...
	message('删除成功',$this->createWebUrl('zjd_record', array('id'=>$id)),'success');
}
if(checksubmit('delete')){
	//批量删除
	$select = $_GPC['select'];
	if(empty($select)){
		message('请选择删除项',$this->createWebUrl("zjd_record",array('id'=>$id,'page'=>$pindex)),"error");
	}
	foreach ($select as $se) {
		pdo_delete($this->lottory_user_table,array('id'=>$se,'rid'=>$rid));
	}
	message('批量删除成功',$this->createWebUrl("zjd_record",array('id'=>$id,'page'=>$pindex)),"success");
}
...
if(checksubmit('down')){
			...
			$up_list = pdo_fetchall("SELECT * FROM ".tablename($this->lottory_user_table)." WHERE  weid=:weid  AND rid=:rid AND  id  IN  ('".implode("','", $_GPC['select'])."')", array(':weid'=>$weid,':rid'=>$rid));
					...
					//导出
						...
						header('Content-Disposition:attachment;filename="砸金蛋中奖名单.xls"');
...
include $this->template('zjd_record');