...
if($op == 'list') {
	$list = pdo_fetchall("SELECT * FROM " . tablename($this->lottory_award_table) . " WHERE weid = :weid AND rid = :rid AND type=:type ORDER BY displayid ASC",array(':weid'=>$weid,':rid'=>$rid,':type'=>'2'));
}elseif ($op == 'post') {
	...
	if (checksubmit('submit')) {
		...
		if (!empty($award_id)) {
			...
			message('保存成功！', $this->createWebUrl('zjd_manage', array('id'=>$id)), 'success');
		} else {
			pdo_insert($this->lottory_award_table,$data);
			message('新增奖品成功！', $this->createWebUrl('zjd_manage', array('id'=>$id,'op' => 'post')), 'success');
		}
	}
}elseif($op=='del'){
	$award_id = intval($_GPC['award_id']);
	...
	message('删除奖品成功！', $this->createWebUrl('zjd_manage', array('id'=>$id)), 'success');
}else {
	message('请求方式不存在');
}

include $this->template('zjd_manage');