	...
	<form class="form-horizontal form" id="form1" action="" method="post" enctype="multipart/form-data">
		...
		<div class="form-group">
			...
			<div class="col-sm-10 col-xs-12">
				...
				<span class="help-block">模块标识符, 应对应模块文件夹的名称, 微赞系统按照此标识符查找模块定义, 只能由字母数字下划线组成 </span>
			</div>
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			...
			<div class="col-sm-10 col-xs-12">
				<input type="text" class="form-control" placeholder="" name="application[url]" value="http://bbs.012wz.com/" />
				<span class="help-block">模块的发布页, 用于发布模块更新信息的页面, 推荐使用微赞模块版块</span>
			</div>
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			...
			<div class="col-sm-10 col-xs-12">
				...
				<?php  } } ?>
				<span class="help-block">订阅特定的消息类型后, 此消息类型的消息到达微赞系统后将会以通知的方式(消息数据只读, 并不能返回处理结果)调用模块的接受器, 用这样的方式可以实现全局的数据统计分析等功能. 请参阅 <a href="http://help.012wz.com">模块消息订阅</a></span>
				<div class="alert-warning alert">注意: 订阅的消息信息是只读的, 只能用作分析统计, 不能更改, 也不能改变微赞处理主流程</div>
			</div>
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			<label class="col-xs-12 col-sm-2 col-md-2 col-lg-1 control-label">兼容的微赞版本</label>
			<div class="col-sm-10 col-xs-12">
				<?php  if(is_array($versions)) { foreach($versions as $v) { ?>
				<label class="checkbox-inline">
					<input name="versions[]" type="checkbox" value="<?php  echo $v;?>" <?php  if(in_array($v, $m['versions'])) { ?> checked="checked"<?php  } ?> />Weizan <?php  echo $v;?>
				</label>
				<?php  } } ?>
				<span class="help-block">当前模块兼容的微赞系统版本, 安装时会判断版本信息, 不兼容的版本将无法安装</span>
			</div>
		</div>
		<div class="form-group">
